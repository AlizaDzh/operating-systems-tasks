...
int main(int argc, char **argv)
{
    ...
    {
        fprintf(stderr, "The file system was not found in '%s' ", path);
    ...
    {
        fputs("There is no root node.", stderr);
        exit_status = EXIT_FAILURE;
        goto cleanup;
    }
    ...
    do
    {
        ...
        {
            fputs("Nonconsecutive link was found.\n", stderr);
            exit_status = KRFFS_Invalid_Link_Error;
            break;
        }

        if ((krffs_is_node_in_file_system(&file_system, node)) == 0)
        {
            fputs("The link that leads outside the file system was found.\n", stderr);
        ...
        {
            fputs("A node with invalid signature was found.\n", stderr);
