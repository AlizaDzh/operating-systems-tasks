...
int main(int argc, char **argv)
{
    ...
    do
    {
        if (cur_node->type == KRFFS_Free_Node)
            printf("Free node\n");
        else if (cur_node->type == KRFFS_Reserved_Node)
    ...
    do
    {
        if (cur_node->type == KRFFS_Free_Node)
        {
            ...
            else if (next_node->type == KRFFS_Reserved_Node)
            {
                ...
                uint64_t cur_node_size = cur_node->size;
                uint64_t prev_cur_node_size = cur_node->previous_node_size;

                memmove((void *)cur_node + 1, (void *)next_node + 1, sizeof(*next_node));
                next_node = cur_node;
                next_node->previous_node_size = prev_cur_node_size;
